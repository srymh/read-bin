...
export type Parse<TResult, TObject extends Record<string, unknown>> = (options: ParseOptions<TObject>) => {
    value: TResult;
    length?: number;
};
export type ParseWithLength<TResult, TObject extends Record<string, unknown>> = (options: ParseOptions<TObject>) => {
    value: TResult;
    length: number;
};
...
export type SchemaEntry<TObject extends Record<string, unknown>, TResult = unknown, TType = Type> = TType extends 'uint8' | 'uint16' | 'uint32' | 'int8' | 'int16' | 'int32' | 'float32' | 'float64' | 'boolean' ? SchemaEntryBase<TType> & {
    /** データのバイト数 */
    byte?: number;
    /** データの個数 */
    count?: 1;
} : TType extends 'string' ? SchemaEntryBase<TType> & {
    /** データのバイト数 */
    byte: number;
    /** データの個数 */
    count?: 1;
} : TType extends 'uint8[]' | 'uint16[]' | 'uint32[]' | 'int8[]' | 'int16[]' | 'int32[]' | 'float32[]' | 'float64[]' | 'boolean[]' ? SchemaEntryBase<TType> & {
    /** データのバイト数 */
    byte?: number | number[];
    /** データの個数 */
    count: number;
    ...
    /** データの個数 */
    count: number;
} : TType extends 'custom' ? (SchemaEntryBase<TType> & {
    /** データのバイト数 */
    byte: number | number[];
    /** データの個数 */
    count: number;
